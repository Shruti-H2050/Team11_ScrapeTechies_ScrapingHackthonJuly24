	...
	public List<RecipeData> extractRecipeData() {
		...
		try {
			...
			for (RecipeData rd : dataList) {
				...
				List<String> prepMethod = new ArrayList<>();
				...
				for (Element p : ingredientsDoc.select("div[id=recipe_small_steps]")) {

					prepMethod.add(p.select("li").text());
				...
				rd.setPreparationMethod(prepMethod);
			}
